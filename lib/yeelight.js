...
exports.discover = function () {
    ...
    var broadcastInterval = setInterval(broadcast, 9000);
    broadcast();
        ...
        .then(result => {
          ...
          if (result !== 'no devices') {
            ...
            Object.keys(yeelights).forEach(function(key) {
              ...
              console.log(added_devices);
              var test = added_devices.hasOwnProperty(yeelights[key].getData().id ? 'waar' : added_devices.hasOwnProperty(yeelights[key].getData().id
              console.log(test);

              // new devices for pairing
              if (result.message_type == 'discover' && !new_devices.hasOwnProperty(result.device.id)) {
