...
exports.discover = function () {
    ...
    var broadcastInterval = setInterval(broadcast, 10000);
    broadcast();
  ...
  advertisements.on('message', (message, address) => {
    process.nextTick(function() {
      parseMessage(message)
        .then(result => {
          console.log('total parsed result');
          console.log(result);
          if (result !== 'no devices') {
            ...
            Object.keys(yeelights).forEach(function(key) {
              ...
              if (result.message_type == 'discover' && !new_devices.hasOwnProperty(result.device.id) && !added_devices.hasOwnProperty(yeelights[key].getData().id)) {
