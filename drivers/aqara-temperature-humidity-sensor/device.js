...
class AqaraTemperatureHumiditySensor extends Device {
  ...
  async onEventFromGateway(device) {
    try {
     ...
     /* measure_humidity */
     if (device.data.humidity) { await this.updateCapabilityValue("measure_humidity", parseFloat(parseFloat(device.data.humidity / 100).toFixed(1))); }
     ...
     /* measure_temperature */
      ...
      /* measure_pressure */
      if (device.data.pressure) { await this.updateCapabilityValue("measure_pressure", parseFloat(parseFloat(device.data.pressure / 100).toFixed(1))); }
