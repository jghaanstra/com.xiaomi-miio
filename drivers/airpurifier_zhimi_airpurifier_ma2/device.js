...
class MiAirPurifierS2Device extends Device {
  ...
  async onInit() {
    try {
      ...
      this.registerCapabilityListener('dim', async (value) => {
        try {
          if (this.miio) {
            return await this.miio.call("set_level_favorite", [this.getFavoriteLevel(speed)]);
          } else {
            this.setUnavailable(this.homey.__('unreachable')).catch(error => { this.error(error) });
            this.createDevice();
            return Promise.reject('Device unreachable, please try again ...');
  ...
  async retrieveDeviceData() {
    try {
      ...
      const result = await this.miio.call("get_prop", ["power", "aqi", "average_aqi", "humidity", "temp_dec", "bright", "mode", "favorite_level", "filter1_life", "use_time", "purify_volume", "led", "buzzer", "child_lock"], { retries: 1 });
...
module.exports = MiAirPurifierS2Device;
