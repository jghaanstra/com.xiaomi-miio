...
class HumidifierDeermaJSQ5Device extends Device {
  ...
  async onInit() {
    try {
      ...
      this.registerCapabilityListener('humidifier_deerma_jsq5_fanlevel', async ( value ) => {
        try {
          if (this.miio) {
  ...
  async handleModeEvent(mode) {
    try {
      if (this.getCapabilityValue('humidifier_deerma_jsq5_fanlevel') !== mode.toString()) {
        const previous_mode = this.getCapabilityValue('humidifier_deerma_jsq5_fanlevel');
        await this.setCapabilityValue('humidifier_deerma_jsq5_fanlevel', mode.toString());
