...
class AqaraButtonAdvanced extends Device {
  ...
  async onEventFromGateway(device) {
    try {
      ...
      if (device && device.data && device.data["status"] === "long_click_release") { await this.homey.flow.getDeviceTriggerCard('click_long_release').trigger(this).catch(error => { this.error(error) }); }
      if (device && device.data && device.data["status"] === "shake") { await this.homey.flow.getDeviceTriggerCard('shake').trigger(this).catch(error => { this.error(error) }); }
