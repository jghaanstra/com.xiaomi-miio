...
class AqaraWleakSensor extends Device {
  ...
  async onEventFromGateway(device) {
    try {
      ...
      /* alarm_water */
      if (device.data.status === "leak") { await this.updateCapabilityValue("alarm_water", true); }
      if (device.data.status === "no_leak") { await this.updateCapabilityValue("alarm_water", false); }
