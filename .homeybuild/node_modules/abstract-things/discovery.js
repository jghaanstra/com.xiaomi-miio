...
const { BasicServiceDiscovery, debug } = require("tinkerhub-discovery");
...
module.exports = class ThingDiscovery extends BasicServiceDiscovery {
  static get type() {
    return "thing";
  }
  ...
  [mapAndAddService](service) {
    Promise.resolve(this[mapper].create(service))
      ...
      .then((mapped) => {
        ...
        if (!mapped.id) {
          this[debug]("Mapped service has no ID:", mapped);
          return;
        }
        ...
        if (mapped instanceof Thing) {
          ...
          const id = mapped.id;
          mapped.on("thing:destroy", () => this.remove(id));
